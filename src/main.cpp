...
*/
const char *ssid = "CONEXAOWEB-G.ALE PROJETOS";
...
void setup()
{
  ...
  server.on("/assets/entireframework.min.css", HTTP_GET, [](AsyncWebServerRequest *request)
            { request->send(LittleFS, "/assets/entireframework.min.css", "text/css"); });
  ...
  server.on("/assets/toggle", HTTP_GET, [](AsyncWebServerRequest *request)
            {
        String status;
